...
# memperbaiki time
df_time = day_df.copy()
df_time['dateday'] = pd.to_datetime(df_time['dateday'])

# Menghitung jumlah total peminjaman (cnt) untuk setiap kombinasi musim dan tahun
pivot_season_year = day_df.groupby(['season', 'year'])['count'].sum().reset_index()
...
with st.sidebar:
    st.header('Davano Al Raffi Abdul Jabbar')
    st.subheader('UNAIR / m180d4ky2427')
    st.image('sepeda.JPG')
...
st.title('Perbandingan Peminjaman Sepeda Berdasarkan Musim dan Tahun')

# Tampilkan dataframe jika diperlukan
st.write('Data Peminjaman Sepeda:')
st.write(pivot_season_year)

# Plot stacked barchart
plt.figure(figsize=(10, 6))
x = pivot_season_year['season'].unique()
y1 = pivot_season_year[pivot_season_year['year'] == '2011']['count']
y2 = pivot_season_year[pivot_season_year['year'] == '2012']['count']
...
def draw_scatterplot(x, y, title):
    plt.figure(figsize=(8, 6))
    sns.scatterplot(x=x, y=y, data=day_df, alpha=0.5)
    plt.title(title)
    plt.xlabel(x)
    plt.ylabel(y)
    st.pyplot()
...
# Gambar scatter plot untuk 'temp' vs 'count'
st.subheader('Temperature vs Count')
draw_scatterplot('temp', 'count', 'Temperature vs Count')

# Gambar scatter plot untuk 'atemp' vs 'count'
st.subheader('Feels Like Temperature vs Count')
draw_scatterplot('atemp', 'count', 'Feels Like Temperature vs Count')

# Gambar scatter plot untuk 'hum' vs 'count'
st.subheader('Humidity vs Count')
draw_scatterplot('hum', 'count', 'Humidity vs Count')



st.caption('Copyright (c) Davano Al Raffi 2024')
